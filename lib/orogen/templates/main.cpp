...
//<% end %>
    ...
    const char * val = ::getenv( "ROCK_TEMPLATE_PREFIX" );
    std::string path = val;
    path+= "../../tools/transport/";
      
    std::vector<RTT::Communication::INameService::shared_ptr> listNameServices;
    typedef void(*setupGenerator_sig)();
    setupGenerator_sig setupGenerator;
    if (ploader->loadPlugins(path)) {
    std::vector<RTT::plugin::PluginLoader::LoadedLib> itLoadedLibs=ploader.get()->loadedLibs;
    
	std::cout << "load plugin success\n";
	// iterate over all plugins
	for(int i=0;i<itLoadedLibs.size();i++)
        {
            // CHeck if the plugin belongs to the rtt_remote category
            if (itLoadedLibs[i].plugname.substr(0,10) == "rtt_remote")
            {
                setupGenerator = (setupGenerator_sig) dlsym(itLoadedLibs[i].handle, "SetupGenerator");
                const char *dlsym_error = dlerror();

                if (dlsym_error) {
                    std::cerr  << "not a valid rtt_remote plugin \n";
                } else {
                    setupGenerator();
                }
            } else {
                std::cerr << "fail to load plugin\n";
            }
            // Check if CreateTaskContextServer methd exist
            typedef RTT::Communication::INameService::shared_ptr(*createNameService_sig)();
            createNameService_sig createNameService;
            if (itLoadedLibs[i].handle != 0) {
                createNameService = (createNameService_sig) dlsym(itLoadedLibs[i].handle, "CreateNameService");
                const char *dlsym_error = dlerror();
                if (dlsym_error) {
                    std::cerr << "this plugin cannot create CreateNameService\n";
                } else {
		    listNameServices.push_back(createNameService());
                    std::cout << "Success: execute CreateNameService()\n";
                }
            } else {
                std::cerr << "No Plugin\n";
            }
...
//First Create all Tasks to be able to set some (slave-) activities later on in the second loop
    ...
    std::auto_ptr<RTT::TaskContext> task_<%= task.name%>(
      ...
      std::vector<std::string> TcsImplementations;
      // Get all implementations of tcs generators
      std::vector<std::string> RegisteredGeneratorImplementations = 
		    RTT::Communication::TaskContextServerFactory::GetInstance()->GetAllRegisteredGeneratorNames();

      RTT::Communication::TaskContextServerCollection tcsCollection;
      for(std::vector<std::string>::const_iterator it=RegisteredGeneratorImplementations.begin();
	it != RegisteredGeneratorImplementations.end();++it)
      {
	  // Get the tcs generator from the TaskContextServerFactory
	  RTT::Communication::ITypeGenerator<RTT::Communication::ITaskContextServer>::shared_ptr typeGenerator = 
	    RTT::Communication::TaskContextServerFactory::GetInstance()->GetGenerator((*it));
	  
	  // Use generator to create task context server
	  RTT::Communication::ITaskContextServer::shared_ptr tascContextServer=typeGenerator->getNewInstance();
	  tcsCollection.add(tascContextServer);
      }
      
      for(int i=0;i<listNameServices.size();i++)
      {
	listNameServices[i]->RegisterTaskContextServer("",tcsCollection);
      }
	
	
    
    
//     tcss[sizeOftcss] = createTaskServer(task_<%= task.name %>.get(),  argc, argv).get();
//     sizeOftcss++;
//     tcss.resize(sizeOftcss+1);
...
//
    ...
    all_peers.concat deployer.each_task.inject(Array.new) {
        |a, m| a.concat m.slaves.map { |s| [m, s] }
    }
...
//     <% end %>
    ...
    for(TaskContextServerCollectionType it=tcsCollection.begin();it!=tcsCollection.end();++it){
	it->Start();
    }
